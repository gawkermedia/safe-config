...
/**
 * A Bootup Errors accumulator. It accumulates errors when applied (<*>). It is also a monad (but does not accumulate
 * errors in that case).
 */
final case class BootupErrors[A] private[BootupErrors] (run: Either[Seq[ConfigError], A]) extends AnyVal {
  ...
  def flatMap[B](f: A => BootupErrors[B]): BootupErrors[B] = BootupErrors(this.run match {
    case Left(e) => Left[Seq[ConfigError], B](e)
...
object BootupErrors {
  ...
  def sequence[A](as: List[BootupErrors[A]]): BootupErrors[List[A]] =
    as.foldRight(BootupErrors(List.empty[A])) { case (a, acc) =>
      for {
        a_ <- a
        acc_ <- acc
      } yield a_ :: acc_
